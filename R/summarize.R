...
summarize_over_group <- function (data_summarized_over_time, group = NULL) {
  ...
  data_summarized_over_group <-
    ...
    summarize(
      ...
      all_cum_cases          = robust_sum(all_cum_cases),
      cap_cum_cases          = robust_sum(all_cum_cases) / pop,
      all_cum_deaths         = robust_sum(all_cum_deaths),
      cap_cum_deaths         = robust_sum(all_cum_deaths) / pop,
      all_cum_tests          = robust_sum(all_cum_tests),
      cap_cum_tests          = robust_sum(all_cum_tests) / pop,
      cap100k_cum_cases      = robust_sum(all_cum_cases) / pop_100k,
      cap100k_cum_deaths     = robust_sum(all_cum_deaths) / pop_100k,
      cap100k_cum_tests      = robust_sum(all_cum_tests) / pop_100k,
