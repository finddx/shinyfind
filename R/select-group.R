...
select_group_server <- function(id, data_r, vars_r) {
  moduleServer(
    ...
    module = function(input, output, session) {
      ...
      # Namespace
      ns <- session$ns
      datamods:::hideUI(selector = paste0("#", ns("reset_all")))
      ...
      observe({
        ...
        for (var in names(rv$data)) {
          if (var %in% rv$vars) {
            datamods:::showUI(id = paste0("container-", var))
          } else {
            datamods:::hideUI(id = paste0("container-", var))
          }
        }
      })
      ...
      observe({
        lapply(
          X = rv$vars,
          FUN = function(x) {
            vals <- sort(unique(strsplit(paste0(unique(rv$data[[x]]), collapse = ";"), split = ";")[[1]]))
          ...
          FUN = function(x) {
            ...
            observeEvent(input[[x]], {
              ...
              if (all(indicator)) {
                ...
                datamods:::showUI(selector = paste0("#", ns("reset_all")))
              }
              ...
              for (i in ovars) {
                if (!isTruthy(input[[i]])) {
                  shinyWidgets::updateVirtualSelect(
                    session = session,
                    inputId = i,
                    choices = sort(unique(strsplit(paste0(unique(data[[i]]), collapse = ";"), split = ";")[[1]]))
                  )
                }
              }
              ...
              if (!isTruthy(input[[x]])) {
                shinyWidgets::updateVirtualSelect(
                  session = session,
                  inputId = x,
                  choices = sort(unique(strsplit(paste0(unique(data[[x]]), collapse = ";"), split = ";")[[1]]))
                )
              }
